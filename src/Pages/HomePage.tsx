...
export default function HomePage() {
  ...
  return (
    <div className="overflow-x-hidden w-full bg-[#0a0a0a] text-[#e8e0d5]" style={{ fontFamily: "'Didact Gothic', sans-serif" }}>
      <style>{`
        ...
        .filters-no-scrollbar::-webkit-scrollbar { display:none; }
      `}</style>

      {/* ── NAV ── */}
      <nav className="fixed top-0 left-0 right-0 z-[100] flex justify-between items-center px-[5%] py-[1.4rem] bg-[#0a0a0a]/92 backdrop-blur-md border-b border-[#151515]">
        <span className="text-[1.3rem] tracking-[0.2em] text-[#e8e0d5]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
          AURA
        </span>
        ...
        <div className="hidden md:flex gap-10">
          {navIds.map((id, i) => (
            <button key={id} onClick={() => scrollTo(id)}
              className="nav-link relative bg-transparent border-0 cursor-pointer text-[#666] hover:text-[#b89a6a] text-[0.7rem] tracking-[0.2em] uppercase transition-colors duration-300"
      ...
      {/* ── MOBILE MENU ── */}
      {menuOpen && (
        <div className="menu-anim fixed inset-0 z-[99] bg-[#0a0a0a]/98 flex flex-col items-center justify-center gap-10">
          {navIds.map((id, i) => (
            <button key={id} onClick={() => scrollTo(id)}
      ...
      {/* ── HERO ── */}
      <section id="inicio" className="min-h-screen flex items-center relative overflow-hidden px-[5%] pt-24 md:pt-0">
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse at 70% 50%, #1a1208 0%, #0a0a0a 60%)" }} />
        <div className="hidden md:block absolute top-[15%] right-[8%] w-px h-[40vh]" style={{ background: "linear-gradient(to bottom, transparent, #b89a6a40, transparent)" }} />

        <div className="relative z-10 max-w-[700px] w-full">
          <p className="fade-up d1 text-[0.7rem] tracking-[0.4em] text-[#b89a6a] uppercase mb-6">
            Nueva colección · 2025
          </p>
          <h1 className="fade-up d2 font-normal leading-[1.05] text-[#e8e0d5] mb-1"
            style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(3rem,12vw,6.5rem)" }}>
            Vestite con
          </h1>
          ...
          <p className="fade-up d4 text-[0.95rem] text-[#888] leading-[1.9] max-w-[420px] mb-12">
        ...
        {/* Float badge */}
        <div className="float-anim hidden md:flex absolute right-[10%] top-1/2 -translate-y-1/2 w-[160px] h-[160px] rounded-full border border-[#b89a6a30] flex-col items-center justify-center bg-[#b89a6a]/5">
          <span className="text-[2rem] text-[#b89a6a]" style={{ fontFamily: "'Playfair Display', serif" }}>✦</span>
          <span className="text-[0.55rem] tracking-[0.25em] text-[#555] uppercase mt-2">Hecho con amor</span>
        </div>

        {/* Scroll hint */}
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
          <span className="text-[0.6rem] tracking-[0.3em] text-[#333] uppercase">Scroll</span>
      ...
      <section id="coleccion" className="py-32 px-[5%] bg-[#0d0d0d]">
        <div className="text-center mb-16">
          <p className="text-[0.65rem] tracking-[0.4em] text-[#b89a6a] uppercase mb-4">Temporada 2025</p>
          <h2 className="font-normal text-[#e8e0d5] mb-6" style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(2rem,4vw,3.5rem)" }}>
            La Colección
          </h2>
        ...
        {/* Filters */}
        <div className="flex gap-2 justify-center md:flex-wrap overflow-x-auto md:overflow-visible filters-no-scrollbar pb-2 md:pb-0 mb-14">
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              className={`shrink-0 border px-5 py-2 text-[0.65rem] tracking-[0.18em] uppercase cursor-pointer transition-all duration-300 whitespace-nowrap
                ${activeCategory === cat
                  ? "border-[#b89a6a] text-[#b89a6a]"
                  : "border-[#222] text-[#666] hover:border-[#b89a6a] hover:text-[#b89a6a]"}`}
              style={{ fontFamily: "'Didact Gothic', sans-serif", background: "transparent" }}>
              {cat}
            </button>
          ))}
        </div>
          ...
          {filtered.map(product => (
            <div key={product.id}
              ...
              <div className="h-[280px] sm:h-[300px] relative flex items-center justify-center"
                ...
                <span className="text-[5rem] text-[#1e1e1e] select-none" style={{ fontFamily: "'Playfair Display', serif" }}>✦</span>
                <div className="absolute inset-0 bg-[#b89a6a]/6 opacity-0 group-hover:opacity-100 transition-opacity duration-400" />
                {product.tag && (
              ...
              <div className="p-5">
                <p className="text-[0.6rem] tracking-[0.2em] text-[#555] uppercase mb-1">{product.category}</p>
                <h3 className="font-normal text-[#e8e0d5] text-[1.1rem] mb-2" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
                  {product.name}
                </h3>
                <p className="text-[0.78rem] text-[#555] leading-[1.7] mb-4">{product.description}</p>
                <div className="flex justify-between items-center border-t border-[#1a1a1a] pt-4">
                  <span className="text-[1.1rem] text-[#b89a6a]" style={{ fontFamily: "'Playfair Display', serif" }}>{product.price}</span>
      ...
      <section id="nosotras" className="py-32 px-[5%] bg-[#0a0a0a] flex items-center gap-16 lg:gap-24 flex-wrap">
        ...
        <div className="relative w-full md:flex-1 min-h-[280px] md:min-h-[400px]">
          ...
          <div className="absolute top-[30px] md:top-[40px] left-[30px] md:left-[40px] w-[180px] md:w-[240px] h-[240px] md:h-[320px] border border-[#b89a6a30] flex items-center justify-center"
            ...
            <span className="text-[5rem] md:text-[6rem] text-[#1e1e18] italic" style={{ fontFamily: "'Playfair Display', serif" }}>A</span>
          </div>
          <div className="absolute bottom-4 right-8 text-right">
            <p className="italic text-[#b89a6a] leading-none" style={{ fontFamily: "'Playfair Display', serif", fontSize: "2.5rem" }}>desde</p>
            <p className="text-[#e8e0d5] leading-none" style={{ fontFamily: "'Playfair Display', serif", fontSize: "3rem" }}>2022</p>
          </div>
        </div>

        {/* Text */}
        <div className="flex-1 min-w-[280px]">
          <p className="text-[0.65rem] tracking-[0.4em] text-[#b89a6a] uppercase mb-5">Nuestra historia</p>
          <h2 className="font-normal text-[#e8e0d5] leading-[1.2] mb-8"
            style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(2rem,3.5vw,3rem)" }}>
            Ropa que<br /><span className="italic text-[#b89a6a]">te define.</span>
          </h2>
          <p className="text-[0.9rem] text-[#666] leading-[2] mb-6">
          ...
          <p className="text-[0.9rem] text-[#666] leading-[2] mb-10">
          ...
          <div className="flex gap-12">
            {[["100%", "Local"], ["✦", "Artesanal"], ["❤", "Con alma"]].map(([val, label]) => (
              <div key={label}>
                <p className="text-[#b89a6a] mb-1" style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.8rem" }}>{val}</p>
                <p className="text-[0.65rem] tracking-[0.2em] text-[#444] uppercase">{label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="contacto" className="py-32 px-[5%] bg-[#0d0d0d] text-center">
        <p className="text-[0.65rem] tracking-[0.4em] text-[#b89a6a] uppercase mb-4">Hablemos</p>
        <h2 className="font-normal text-[#e8e0d5] mb-2" style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(2rem,4vw,3.5rem)" }}>
          Encontranos en
        </h2>
        <h2 className="italic text-[#b89a6a] mb-12" style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(2rem,4vw,3.5rem)" }}>
          nuestras redes
        </h2>
        ...
        <div className="flex gap-4 justify-center flex-wrap mb-16">
          {contacts.map(({ label, handle, icon, link }) => (
            <a key={label} href={link} target="_blank" rel="noopener noreferrer"
              className="group border border-[#1a1a1a] hover:border-[#b89a6a] bg-[#111] hover:bg-[#131108] p-8 min-w-[200px] sm:min-w-[220px] cursor-pointer transition-all duration-300 no-underline flex flex-col items-center">
              <p className="text-[1.8rem] text-[#b89a6a] mb-3" style={{ fontFamily: "'Playfair Display', serif" }}>{icon}</p>
              <p className="text-[0.6rem] tracking-[0.2em] text-[#444] uppercase mb-1">{label}</p>
              <p className="text-[0.85rem] text-[#888]">{handle}</p>
            </a>
          ))}
        </div>

        <p className="text-[0.75rem] text-[#333] tracking-[0.1em]">
      ...
      <footer className="border-t border-[#151515] px-[5%] py-8 flex flex-col sm:flex-row justify-between items-center gap-2">
        <span className="text-[1rem] tracking-[0.2em] text-[#2a2a2a]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>AURA</span>
        <span className="text-[0.6rem] tracking-[0.15em] text-[#2a2a2a] uppercase">Hecho con amor · {new Date().getFullYear()}</span>
      </footer>
    </div>
  );
}